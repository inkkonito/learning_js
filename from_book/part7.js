...
console.log(blues.every(function(e) {
     return (e.indexOf("Bleu") >= 0)
})) // all items contain "Blue

console.log(blues2.every(function(e) {
    return regex.test(e);
...
console.log(pairs.filter(function(val) {
    return (val%2==0)
}));
...
console.log(shortNames.filter(function(val) {
    return val.length<=4 // return only names Max, Jean, Paul because length is <= 4
}))

// find

console.log(shortNames.find(function(val) {
    return val.charAt(0) === "J" // return Jean (first J)
}))
...
console.log(shortNames.findIndex(function(val) {
